...
- `C-k` — `move pane up (passes C-k to Neovim when open)`
- `C-l` — `move pane right (passes C-l to Neovim when open)`
- `C-o` — `send-keys -X copy-pipe-and-cancel "sed s/##/####/g | xargs -I {} tmux send-keys 'nvim -- "{}"'; tmux send-keys 'C-m'"`
...
- `Y` — `send-keys -X copy-pipe-and-cancel pbcopy`
- `n` — `run-shell "tmux send-keys -X search-again; copycat_jump.sh 'next'; true"`
- `o` — `send-keys -X copy-pipe-and-cancel "sed s/##/####/g | xargs -I {} tmux run-shell -b 'cd #{pane_current_path}; open "{}" > /dev/null'"`
- `q` — `send-keys -X cancel`
- `v` — `send-keys -X begin-selection`
- `y` — `send-keys -X copy-pipe-no-clear pbcopy`
